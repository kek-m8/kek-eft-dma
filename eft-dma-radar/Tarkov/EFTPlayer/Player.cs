...
namespace eft_dma_radar.Tarkov.EFTPlayer
{
    ...
    public abstract class Player : IWorldEntity, IMapEntity, IMouseoverEntity, IESPEntity, IPlayer
    {
        ...
        /// </summary>
        public virtual eft_dma_shared.Common.Players.Skeleton Skeleton_ => throw new NotImplementedException(nameof(Skeleton_));
        ...
        public virtual ref Vector3 Position => ref this.Skeleton_.Root.Position;
        ...
        public void Draw(SKCanvas canvas, LoneMapParams mapParams, ILocalPlayer localPlayer)
        {
            try
            {
                ...
                else
                {
                    ...
                    if (this == localPlayer)
                        return;
                    if(Config.MapFollowTeammate == 1)
                    {
                        foreach (var x in Memory.Players)
                        {
                            if (Memory.LocalPlayer.GroupID == x.GroupID && Memory.LocalPlayer.AccountID != x.AccountID && x.IsAlive && !x.HasExfild && x.IsFriendlyActive && x.IsPmc)
                            {
                                height = Position.Y - x.Position.Y;
                                dist = Vector3.Distance(x.Position, Position);
                            }
                        }
                    }
                    else
                    {
                        height = Position.Y - localPlayer.Position.Y;
                        dist = Vector3.Distance(localPlayer.Position, Position);
                    }
        ...
        public void DrawESP(SKCanvas canvas, LocalPlayer localPlayer)
        {
            ...
            if (dist > Program.Config.MaxPlayerDistance && this.IsPmc)
                return;
            if (dist > Program.Config.MaxDistance && !this.IsPmc)
                return;
