...
namespace eft_dma_radar.UI.Radar
{
    public sealed partial class MainForm : Form
    {
        ...
        public FollowIndexType followIndex;
        ...
        public enum FollowIndexType : int
        {
            FOLLOW_LOCALPLAYER = 0,
            FOLLOW_DUO = 1,
            FOLLOW_FREE = 2,
        };
        ...
        private void Radar_PaintSurface(object sender, SKPaintGLSurfaceEventArgs e)
        {
            ...
            next:
                ...
                if (inRaid && localPlayer is not null) // LocalPlayer is in a raid -> Begin Drawing...
                {
                    ...
                    Player teamMate = null; Vector3 teamPlayerPos = new Vector3(); Vector2 teamPlayerMapPos = new Vector2();
                    if (followIndex is FollowIndexType.FOLLOW_DUO)
                    {
                        foreach (var player in Memory.Players)
                        {
                            if (Memory.LocalPlayer.GroupID == player.GroupID && Memory.LocalPlayer.AccountID != player.AccountID && player.IsAlive && !player.HasExfild && player.IsFriendlyActive && player.IsPmc)
                            {
                                teamMate = player;
                            }
                        }
                    ...
                    {
                        teamPlayerPos = teamMate.Position;
                        teamPlayerMapPos = teamPlayerPos.ToMapPos(map.Config);
                    }
                    ...
                    LoneMapParams mapParams = new LoneMapParams(); // Drawing Source
                    switch (followIndex)
                    {
                        case FollowIndexType.FOLLOW_FREE: // map fixed location, click to pan map
                            mapParams = map.GetParameters(skglControl_Radar, _zoom, ref _mapPanPosition);
                            break;
                        case FollowIndexType.FOLLOW_LOCALPLAYER: // map auto follow local player
                            mapParams = map.GetParameters(skglControl_Radar, _zoom, ref localPlayerMapPos);
                            break;
                        case FollowIndexType.FOLLOW_DUO: // map auto follow team player
                            if (teamMate is null) { mapParams = map.GetParameters(skglControl_Radar, _zoom, ref localPlayerMapPos); break; }
                            mapParams = map.GetParameters(skglControl_Radar, _zoom, ref teamPlayerMapPos);
                            break;
                    ...
                    if (Config.QuestHelper.Enabled && localPlayer.IsPmc)
                    {
    ...
    "Sets the lines scaling factor for the ESP Window.\nIf you are rendering at a really high resolution, you may want to increase this.");
        ...
        private void LoadESPConfig()
        {
            ...
            checkBox_ESP_GrenadeShow.Checked = Config.ESP.ShowGrenades;
            checkBox_ESP_GrenadeText.Checked = Config.ESP.ShowGrenadeName;
            checkBox_ESP_GrenadeIcon.Checked = Config.ESP.ShowGrenadeIcons;
            checkBox_ESP_TripwriteShow.Checked = Config.ESP.ShowTripwires;
            checkBox_ESP_TripwireText.Checked = Config.ESP.ShowTripwireName;
            checkBox_ESP_TripwireLine.Checked = Config.ESP.ShowTripwireLine;
...
#pragma warning restore CS8625 // Cannot convert null literal to non-nullable reference type.
        ...
        private void ContainersUnSelectAll()
        {
            for (int i = 0; i < checkedListBox_Containers.Items.Count; i++)
            {
        ...
        private void ContainersSelectAllStashes()
        {
            for (int i = 0; i < checkedListBox_Containers.Items.Count; i++)
            {
                if (checkedListBox_Containers.Items[i].ToString().Contains("cache", StringComparison.CurrentCultureIgnoreCase) || checkedListBox_Containers.Items[i].ToString().Contains("body", StringComparison.CurrentCultureIgnoreCase))
                {
                    checkedListBox_Containers.SetItemChecked(i, true);
                }
            }
        ...
        {
            Config.ESP.ShowOnlyWishlist = checkBox_ShowOnlyWishlist.Checked;
            LootFilter.ShowWishlist = checkBox_ShowOnlyWishlist.Checked;
            LootFilter.ShowFood = checkBox_ShowOnlyWishlist.Checked ? false : checkBox_ShowFood.Checked;
            LootFilter.ShowMeds = checkBox_ShowOnlyWishlist.Checked ? false : checkBox_ShowMeds.Checked;
            LootFilter.ShowBackpacks = checkBox_ShowOnlyWishlist.Checked ? false : checkBox_ShowBackpacks.Checked;
            _lootMenuTimer.Restart();
            Config.Save();
        }

        private void flowLayoutPanel_Loot_Paint(object sender, PaintEventArgs e)
        {
        ...
        {
            followIndex++;
            if (followIndex > FollowIndexType.FOLLOW_FREE)
            {
                followIndex = FollowIndexType.FOLLOW_LOCALPLAYER;
            }
            switch (followIndex)
            {
                case FollowIndexType.FOLLOW_LOCALPLAYER:
                    Config.MapFollowTeammate = 0;
                    button1.Text = $"Map Follow Duo";
                    Config.Save();
                    break;
                case FollowIndexType.FOLLOW_DUO:
                    Config.MapFollowTeammate = 1;
                    button1.Text = $"Map Free";
                    Config.Save();
                    break;
                case FollowIndexType.FOLLOW_FREE:
                    Config.MapFollowTeammate = 2;
                    button1.Text = $"Map Follow Me";
        ...
        {
            bool selectAllStashes = checkBox1.Checked;
            Config.Containers.SelectAllStashes = selectAllStashes;
            if (selectAllStashes)
            {
        ...
        {
            Config.ESP.ShowGrenades = checkBox_ESP_GrenadeShow.Checked;
            checkBox_ESP_GrenadeIcon.Enabled = checkBox_ESP_GrenadeShow.Checked;
            checkBox_ESP_GrenadeText.Enabled = checkBox_ESP_GrenadeShow.Checked;
        }

        private void checkBox_ESP_GrenadeIcon_CheckedChanged(object sender, EventArgs e)
        {
            Config.ESP.ShowGrenadeIcons = checkBox_ESP_GrenadeIcon.Checked;
        }

        private void checkBox_ESP_GrenadeText_CheckedChanged(object sender, EventArgs e)
        {
            Config.ESP.ShowGrenadeName = checkBox_ESP_GrenadeText.Checked;
        }

        private void checkBox_ESP_TripwriteShow_CheckedChanged(object sender, EventArgs e)
        {
            Config.ESP.ShowTripwires = checkBox_ESP_TripwriteShow.Checked;
            checkBox_ESP_TripwireIcon.Enabled = checkBox_ESP_TripwriteShow.Checked;
            checkBox_ESP_TripwireText.Enabled = checkBox_ESP_TripwriteShow.Checked;
            checkBox_ESP_TripwireLine.Enabled = checkBox_ESP_TripwriteShow.Checked;
        }

        private void checkBox_ESP_TripwireText_CheckedChanged(object sender, EventArgs e)
        {
            Config.ESP.ShowTripwireName = checkBox_ESP_TripwireText.Checked;
        }

        private void checkBox_ESP_TripwireLine_CheckedChanged(object sender, EventArgs e)
        {
            Config.ESP.ShowTripwireLine = checkBox_ESP_TripwireLine.Checked;
        }
